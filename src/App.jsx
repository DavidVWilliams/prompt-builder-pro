...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans flex flex-col md:flex-row">
      ...
      {/* Sidebar / Tools Panel */}
      <div className="w-full md:w-80 bg-white border-r border-slate-200 flex flex-col shadow-sm z-10 h-auto md:h-screen sticky top-0 overflow-y-auto">
          ...
          <div className="space-y-2">
            {tools.map((tool) => (
              ...
              >
                ...
                <p className="text-xs text-slate-500 ml-[44px] leading-relaxed">{tool.desc}</p>
              </button>
            ))}
          </div>
        </div>

        <div className="p-4 bg-slate-50 border-t border-slate-200 m-4 rounded-xl">
          <div className="flex items-start gap-2 text-indigo-600 mb-2">
            <MessageSquareQuote className="w-4 h-4 mt-0.5 opacity-70" />
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <div className="flex-1 overflow-auto p-4 md:p-6 bg-slate-50 flex flex-col lg:flex-row gap-6">
          ...
          <div className="flex-1 flex flex-col bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden min-h-[400px]">
            ...
            <textarea
              className="flex-1 w-full p-4 md:p-6 resize-none focus:outline-none text-slate-700 leading-relaxed"
            ...
            <div className="p-4 border-t border-slate-100 bg-white flex justify-end shrink-0">
              <button
                ...
                className="flex items-center gap-2 bg-indigo-600 text-white px-5 py-2.5 rounded-xl font-medium hover:bg-indigo-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-sm"
              >
          ...
          {/* AI Output Column */}
          <div className="flex-1 flex flex-col bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden min-h-[400px]">
             <div className="bg-indigo-50 border-b border-indigo-100 px-4 py-2 flex justify-between items-center shrink-0">
              <span className="text-xs font-semibold text-indigo-800 uppercase tracking-wider">AI Assistant Feedback</span>
              {aiResponse && (
                <button 
                  onClick={() => copyToClipboard(aiResponse)}
                  className="text-indigo-400 hover:text-indigo-700 transition-colors flex items-center gap-1 text-xs font-medium"
                >
